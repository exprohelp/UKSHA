...
namespace UKSHAApi.Repository.ApplicationResources
{
	public class Admin
	{
		public dataSet Authenticate_LoginInfo(Authenticate objBO)
		{
			...
			{
				using (SqlCommand cmd = new SqlCommand("pSHA_Authenticate_LoginInfo", con))
				{
		...
		public dataSet MasterQueries(ipAdmin objBO)
		{
			...
			using (SqlConnection con = new SqlConnection(GlobalConfig.ConStr_UKSHA))
			{
				using (SqlCommand cmd = new SqlCommand("pSHA_MasterQueries", con))
				{
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.CommandTimeout = 2500;
		...
		public string InsertUpdatemaster(MasterInfo objBO)
		{
			...
			using (SqlConnection con = new SqlConnection(GlobalConfig.ConStr_UKSHA))
			{
				using (SqlCommand cmd = new SqlCommand("pSHA_m_InsertUpdatemaster", con))
				{
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.CommandTimeout = 2500;
