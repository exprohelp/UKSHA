...
namespace UKSHAApi.Repository.MobileApp
{
	public class DownloadLogic
	{
        public string DownloadLISReport(PatientInfo ipProfile)
        {
            ...
            try
            {
                ...
                Url = "http://192.168.0.21/Chandan/Design/Lab/labreportnew_NhmSha.aspx?IsPrev=1&PHead=1&testid=" + ipProfile.TestIds + "&Mobile=1";                
                if (rs.ResultSet.Tables[0].Rows.Count > 0)
                {
                    if (rs.ResultSet.Tables[0].Rows[0]["DBType"].ToString() == "BackupDB")
                    {
                        Url = "http://192.168.0.21/Chandan/Design/Lab/labreportnew_NhmShaArchive1.aspx?IsPrev=1&PHead=1&testid=" + ipProfile.TestIds + "&Mobile=1";
                    }
                }
